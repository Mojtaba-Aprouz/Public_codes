...
def conv(image, kernel):
    ...
    for row in range(Hi):
        for col in range(Wi):
                    out[row,col]=np.sum( padded[row:row+Hk , col:col+Wk]*kernel)
    ### END YOUR CODE
...
def gaussian_kernel(size, sigma):
    ...
    ### YOUR CODE HERE
    x,y=np.mgrid[-delta:delta+1,-delta:delta+1]
    norm=1/(2.0*np.pi*(sigma**2))
    filter_values=np.exp(-((x**2+y**2)/(2.0*sigma**2)))*norm
...
def non_maximum_suppression(G, theta):
    ...
    pad_width1 = 1
    pad_width = ((pad_width0,pad_width0),(pad_width1,pad_width1))
    padded = np.pad(G, pad_width, mode='edge')

    for i in range(1,H+1):
      for j in range(1,W+1):
        if theta[i-1,j-1] == 0 or theta[i-1,j-1] == 180  or theta[i-1,j-1] == 360:
...
def double_thresholding(img, high, low):
    ...
    for i in range(img.shape[0]):
        for j in range(img.shape[1]):
            if high < img[i,j]:
                strong_edges[i,j]=1
            elif (low < img[i,j] and high > img[i,j]) :
                weak_edges[i,j]=1
...
def link_edges(strong_edges, weak_edges):
    ...
    labeled=label(strong_edges+weak_edges, background=0, connectivity=2)
    unique, counts = np.unique(labeled, return_counts=True)
    for i in range(H):
      for j in range(W):
        if strong_edges[i,j]>0:
          k=labeled[i,j]
          labeled[labeled==k]=max(unique)+1
          
    edges,temp=double_thresholding(labeled, max(unique),0)
...
def hough_transform(img):
    ...
    for i in range(len(xs)):
      for j in (range(num_thetas)):
        rho = diag_len + int(round(xs[i] * cos_t[j] + ys[i] * sin_t[j]))
        accumulator[rho,j]+=1
